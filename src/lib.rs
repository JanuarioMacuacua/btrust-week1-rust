...
pub const CURRENT_BLOCK_HEIGHT: u64 = 890_000;
...
pub fn generate_address(prefix: &str) -> String {
    ...
    for _ in 0..suffix_len {
        ...
        let idx = (seed >> 16) % 36;
        suffix.push(chars[idx as usize]);
    }
...
pub fn validate_block_height(height: i64) -> (bool, String) {
    ...
    if height < 0 {
        (false, String::from("Block height cannot be negative"))
    } else if height > 1_000_000 {
        (false, String::from("Block height is unrealistic"))
    } else {
...
pub fn find_utxo_with_min_value(utxos: &[Utxo], target: u64) -> Option<Utxo> {
    ...
    let mut valid_utxos: Vec<&Utxo> = utxos.iter().filter(|utxo| utxo.value >= target).collect();
    if valid_utxos.is_empty() {
        return None;
    }
    valid_utxos.sort_by_key(|utxo| utxo.value);
    Some(valid_utxos[0].clone())
}
...
pub fn create_utxo(
    ...
    base.insert(String::from("txid"), String::from(txid));
    base.insert(String::from("vout"), vout.to_string());
...
/// Extract the version number from a raw transaction hex string.
/// The version is the first 4 bytes in little-endian order.
pub fn extract_tx_version(raw_tx_hex: &str) -> Result<u32, String> {
    // TODO: Convert hex string to bytes, take first 4 bytes, interpret as u32 little-endian
    if raw_tx_hex.len() < 8 {
